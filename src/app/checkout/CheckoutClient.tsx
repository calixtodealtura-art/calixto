...
if (!MP_PUBLIC_KEY) {
  console.error(
    'Falta NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY: el checkout con Mercado Pago no va a funcionar.'
  )
} else {
...
const ZIP_PATTERN   = '[A-Za-z]?\\d{4}[A-Za-z]{0,3}'
const ZIP_HINT      = 'Código postal argentino, ej: 1425 o C1425AAA'
const PHONE_PATTERN = '[0-9+\\-\\s()]{6,20}'
const PHONE_HINT    = 'Solo números, espacios y + - ( )'
...
const ADDRESS_FIELDS: FormField[] = [
  ...
  { name: 'zipCode',  label: 'Código postal',     type: 'text', pattern: ZIP_PATTERN,   hint: ZIP_HINT   },
  { name: 'phone',    label: 'Teléfono',          type: 'tel',  pattern: PHONE_PATTERN, hint: PHONE_HINT },
]
              ...
              <form onSubmit={handleConfirm} className="space-y-5">
                ...
                {isPickup
                  ? PICKUP_FIELDS.map(field => (
                      <div key={field.name}>
                        <label htmlFor={`pickup-${field.name}`} className="block text-[11px] tracking-[0.15em] uppercase text-green-olive mb-1.5 font-light">
                          {field.label}
                        </label>
                        <input
                          id={`pickup-${field.name}`}
                          type={field.type}
                          name={field.name}
                          required
                        ...
                        />
                        {field.hint && (
                          <p className="text-[10px] text-gray-400 font-light mt-1">{field.hint}</p>
                        )}
                      </div>
                    ))
                  : ADDRESS_FIELDS.map(field => (
                      <div key={field.name}>
                        <label htmlFor={`address-${field.name}`} className="block text-[11px] tracking-[0.15em] uppercase text-green-olive mb-1.5 font-light">
                          {field.label}
                        </label>
                        <input
